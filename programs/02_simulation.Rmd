...
df_linear = rbind(linear_secom1, linear_secom2, linear_prop, 
...
df_linear_fig1$method = factor(df_linear_fig1$method, 
                               levels = c("SECOM (Thresholding)", "SECOM (Filtering)",
                                          "Proportionality", "SparCC", 
...
p_linear2 = df_linear_fig2 %>%
  ...
  labs(title = "Sparsity Recovery", x = NULL, y = NULL) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),
        strip.background = element_rect(fill = "white"))
...
col_lab = c("tpr", "fpr")
colnames(nonlinear_secom) = col_lab
...
nonlinear_secom = nonlinear_secom %>%
  mutate(method = "SECOM", 
         setting = rep(simpattern$setting, each = iter_num))
...
df_nonlinear = rbind(nonlinear_secom, nonlinear_prop, nonlinear_sparcc, 
...
df_nonlinear_fig$method = factor(df_nonlinear_fig$method, 
                                 levels = c("SECOM", "Proportionality", "SparCC", 
                                            "SPIEC-EASI (MB)", "SPIEC-EASI (GL)", "Pearson"))
...
p_nonlinear = df_nonlinear_fig %>%
  ...
  scale_fill_manual(name = NULL, values = c("#1B9E77", "#7570B3", "#E7298A",
...
# 3. Synthetic data: two ecosystems {.tabset}
...
## 3.4 Standard Pearson correlation coefficient
...
complex_se = read_csv("../outputs/complex/complex_se.csv")
complex_se1 = complex_se[, 1:4]
complex_se2 = complex_se[, 5:8]
...
df_complex = rbind(complex_secom1, complex_secom2, complex_prop, 
