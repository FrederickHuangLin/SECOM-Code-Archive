...
secom_compare = linear_secom[, 13:16]
...
## 1.9 Compare SECOM results between Pearson and Distance measures

* (1, 1): Pearson = TRUE, distance = TRUE
    
* (1, 0): Pearson = TRUE, distance = FALSE

* (0, 1): Pearson = FALSE, distance = TRUE

* (0, 0): Pearson = FALSE, distance = FALSE

```{r}
secom_compare = secom_compare %>%
  mutate(setting = rep(simpattern$setting, each = iter_num))

secom_compare2 = secom_compare %>%
  group_by(setting) %>%
  summarise_all(mean) %>%
  mutate_if(is.numeric, function(x) round(x))

colnames(secom_compare2) = c("Setting", "(1, 1)", "(1, 0)", "(0, 1)", "(0, 0)")
secom_compare2$Setting = factor(secom_compare2$Setting, 
                                levels = c("50, 100, 0.5", "50, 100, 2",
                                           "100, 200, 0.5", "100, 200, 2"))
secom_compare2 = secom_compare2 %>%
  arrange(Setting)
datatable(secom_compare2)
```
...
secom_compare = nonlinear_secom[, 7:10]
...
## 2.9 Compare SECOM results between Pearson and Distance measures
